...
///
/// In SEAL's BGV encryption scheme, the parameter `bits` typically refers to the bit-size of the modulus.
...
///
/// ## Range in SEAL:
/// In SEAL, the choice of `bits` often depends on a trade-off between security and performance.
/// Larger bit-sizes generally offer more security but might be less efficient in terms of computation.
/// Users should refer to SEAL's official documentation or relevant publications for detailed guidelines on selecting `bits`.
///
/// # Example
///
/// ```
