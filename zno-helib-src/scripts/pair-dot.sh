...
for file1 in "$dir_src"/*$pat1; do
    # Determine prefix and second file of the pair
    prefix=$(basename "$file1" $pat1)
    file2="${dir_src}/${prefix}${pat2}"

    if [[ -f "$file2" ]]; then
        merged_file="$dir_dst/${prefix}.dot"
        ...
        sed -i 's/height=[^,]*,//g' "$merged_file"
        sed -i 's/width=[^,]*,//g' "$merged_file"
        sed -i 's/fontsize="[^"]*",//g' "$merged_file"
        sed -i 's/URL="[^"]*",//g' "$merged_file"
        sed -i 's/,,/,/g' "$merged_file"
        sed -i 's/Node\([0-9]\+\)/N\1/g' "$merged_file"

        # Rename the file with the number of characters
        mv "$merged_file" "$dir_dst/$(wc -c < "$merged_file")__${prefix}.dot"
    fi
done

# Merge files while considering the token limit
token_limit=4000
current_token_count=0
merge_file_count=0
merge_file="$dir_dst/merged_${merge_file_count}.dot"
for file in "$dir_dst/"*.dot; do
    file_token_count=$(wc -c < "$file")

    # Assuming 1 token is approximately 4 characters
    file_token_count=$((file_token_count / 3))

    if [[ $file_token_count -gt $token_limit ]]; then
        # If file on its own exceeds token limit, rename and continue
        mv "$file" "$dir_dst/${file_token_count}__1__$file"
        continue
    fi

    if [[ $(($current_token_count + $file_token_count)) -gt $token_limit ]]; then
        # If adding the current file exceeds token limit, rename merged file and reset counters
        mv "$merge_file" "$dir_dst/${current_token_count}__${merge_file_count}__merged.dot"
        current_token_count=0
        merge_file_count=$((merge_file_count+1))
        merge_file="$dir_dst/merged_${merge_file_count}.dot"
    fi

    # Append the current file to the merged file
    cat "$file" >> "$merge_file"
    rm "$file"
    current_token_count=$(($current_token_count + $file_token_count))
done

# Cleanup temporary files
find "${dir_dst}" -name "*__.dot" -delete
...
# # Merging without exceeding token limit
# counter=1
# tokens=0
# output_file="${dir_dst}/merged_${counter}.dot"

# for file in "${dir_dst}"/*.dot; do
#     chars=$(wc -c < "$file")

#     # Start new file if token limit is exceeded
#     if (( tokens + chars > 4000 )); then
#         counter=$((counter + 1))
#         tokens=0
#         output_file="${dir_dst}/merged_${counter}.dot"
#     fi

#     # If single file exceeds the token limit, skip it
#     if (( chars < 4000 )); then
