...
fn find_lib(target: &str) -> (Vec<PathBuf>, PathBuf) {
    #[cfg(feature = "vendored")]
    {
        // vendor if the feature is present, unless
        // LIB_NO_VENDOR exists and isn't `0`
        if env("LIB_NO_VENDOR").map_or(true, |s| s == "0") {
            return find_vendored::get_lib(target);
        }
    }
    find_normal::get_lib(target)
}

fn check_fhe_kind() {
    if cfg!(feature = "helib") {
        println!("cargo:rustc-cfg=helib");
        println!("cargo:helib=true");
    }
}
...
fn main_old() -> miette::Result<()> {
    ...
    if !Path::new(&include_dir).exists() {
        panic!(
            "HElib include directory does not exist: {}",
    ...
    println!("cargo:include={}", include_dir.clone().to_string_lossy());

    // Run autocxx
    postprocess(&[include_dir.clone()]);
...
fn postprocess(include_dirs: &[PathBuf]) {
    ...
    let mut b = autocxx_build::Builder::new("src/main.rs", include_dirs)
        .auto_allowlist(true)
        .build().unwrap();
        // This assumes all your C++ bindings are in main.rs
    b.flag_if_supported("-std=c++17")
...
//     // Take a look at
//     // `version.h` to see what version HElib claims to be.
//     //
//     // To handle conditional compilation we slurp up the configuration
