...
// This module provides Rust bindings and wrappers for the C++ HElib library.
//
...
impl cxx::ExternType for Context {
    type Id = cxx::type_id!("helib::Context");
    type Kind = cxx::kind::Trivial;
}

/// Represents a handle to an `EncryptedArray` from the C++ HElib library.
///
...
/// Represents a const handle to an `EncryptedArray` from the C++ HElib library.
///
/// The underlying `UniquePtr` to the C++ object is automatically cleaned up when
/// this struct goes out of scope, ensuring proper resource management.
