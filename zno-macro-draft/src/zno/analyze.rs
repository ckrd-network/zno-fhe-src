...
// Zno Attribute Features
//
// The feature set for the `zno` attribute is evolving, heading to a 1.0
...
//   Current default:
//
//   - `#[zno] async fn` creates thread-safe span (`Span`)
//         - `#[zno(enter_on_poll = true)] async fn` creates local context
//           span (`LocalSpan`)
//   - `#[zno] fn` create local context span (`LocalSpan`)
//
...
// `Zno` should be moved into `minitrace-macro::validate`.
// Implement `syn::Parse` there, so that in `lib.rs`:
//
//    let attr_args = parse_macro_input!(argsc as crate::zno::validate::TraceAttr);
//    let itemfn = parse_macro_input!(itemc as ItemFn);
//    let args2: proc_macro2::TokenStream = args.clone().into();
//    zno::validate(args2, item.into());
//    let model = zno::analyze(attr_args, itemfn);
//
// becomes
//
//    use crate::zno::validate::Zno;
//    let zno = parse_macro_input!(argsc as Zno);
//    let item = parse_macro_input!(itemc as Zno);
//    let model = zno::analyze(zno, item);
#[derive(
    Clone,
    std::fmt::Debug,
    PartialEq,
    ...
    //
    // ```
    // fn from_list(items: &[NestedMeta]) -> Result<Zno, syn::Error>
    // ```
    //
...
/// A struct representing the parsed attributes of the `#[zno]` macro.
///
/// # Arguments
///
...
///
/// ```
/// #[zno(name = "foo", scope = Scope::Thread, enter_on_poll = true,
...
/// Analyzes the provided `Zno` and `TokenStream` and produces a `Models` object.
///
/// This function visits each function in the `TokenStream` and merges it with its zno settings.
/// The zno settings are determined by the `Zno` object.
///
/// # Arguments
///
/// * `zno`: A `Zno` object that holds the attribute parameters.
/// * `items`: A `TokenStream` that represents the items the `#[zno]` attribute is applied to.
///
/// # Examples
///
/// ```
...
/// };
///
/// let models = analyze(zno, items.into());
/// ```
///
/// # Panics
///
...
pub fn analyze(
    ...
    // Prepare and merge each ItemFn with its zno settings
    ...
    for f in visitor.functions {
        ...
        let traced_item = if let crate::zno::Zno {
            default: _,
            validated: _,
            name,
            scope: Some(scope),
        ...
        } = zno.clone()
        {
            ...
            // NOTE:
            //     `#[zno(key = "value")]` maps to
            //     `#[zno(name = "__default", key = "value")]`
...
///
/// This struct is used to hold the parsed attributes from the `#[zno(...)]` macro and the function that the macro is applied to.
...
///     scope: crate::zno::parse::Scope::Thread,
...
pub struct TracedItem {
    // These are the fields parsed as AttributeArgs into the `Zno` struct
    pub name: syn::LitStr,
    pub scope: crate::zno::parse::Scope, // Scope::Local, Scope::Thread, etc.
    ...
    // `item_fn` pairs each function with the `#[zno(...)]` settings.
...
///
/// This enum is used to represent either an `Attribute` or an `Item`. The `Attribute` variant holds a `Zno` object,
...
///
/// ```
/// // Assuming `zno` is a Zno object and `traced_item` is a TracedItem object
/// let model_attribute = Model::Attribute(zno);
...
/// `#[zno(recurse=all|public|private)]` is applied on a function or, eventually,
/// a module.
///
...
/// match model {
///     Model::Attribute(attribute) => assert_eq!(attribute, Zno::default()),
...
/// # Panics
///
/// This function does not panic under normal conditions. However, it may panic if there's an issue with the `Default` implementation for `Zno`.
///
/// # Arguments
///
...
/// Provides a `Default` implementation for `Zno`.
///
/// This implementation creates a `Zno` object with default values. The `name`, `scope`, `enter_on_poll`, `recorder`, `recurse`, `root`, `variables`, `parent`, and `async_trait` fields are all set to some default value.
///
/// # Examples
///
/// ```
/// let zno = Zno::default();
/// assert_eq!(zno.name, Some(syn::LitStr::new("__default", proc_macro2::Span::call_site())));
/// assert_eq!(zno.scope, Some(Scope::Local));
/// assert_eq!(zno.enter_on_poll, Some(syn::LitBool::new(false, proc_macro2::Span::call_site())));
...
/// assert_eq!(traced_item.scope, crate::zno::parse::Scope::Local);
...
impl Default for TracedItem {
    fn default() -> Self {
        ...
        let scope = crate::zno::parse::Scope::Local;
...
mod tests {
    ...
    use crate::zno::analyze::Model;
    use crate::zno::analyze::Models;
    ...
    fn with_traces() {
        ...
        let models = analyze(zno, items.clone());
