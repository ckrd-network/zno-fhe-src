...
//
// The input stream comes from the `zno::validate::validate` function.
// The output vector goes to the `zno::analyze::analyze` function.
...
///
/// The `Zno` struct contains fields for different attributes of a traced item, such as its name, whether it's validated, whether it enters on poll, its scope, parent, recorder, whether it recurses, whether it's a root, its variables, and whether it's an async trait or function.
///
/// # Examples
///
/// ```
...
/// };
/// assert_eq!(zno.name.value(), "my_trace");
/// ```
///
/// # Safety
///
...
impl syn::parse::Parse for Zno {
    /// Implementation of the `syn::parse::Parse` trait for the `Zno` struct.
    ///
    /// This implementation allows a `Zno` object to be parsed from a `syn::parse::ParseStream`.
    ///
    /// # Examples
    ///
    /// ```
    /// // Assuming `input` is a syn::parse::ParseStream containing valid data for a Zno
    /// let zno = syn::parse::Parse::parse(input).unwrap();
    /// assert_eq!(zno.name.value(), "my_trace");
    /// ```
    ///
    /// # Errors
    ...
    /// # Arguments
    ///
    /// `input` - A `syn::parse::ParseStream` from which to parse a `Zno`.
    fn parse(input: syn::parse::ParseStream) -> syn::Result<Self> {
        ...
        if arg_n > 3 {
            // tests/zno/ui/err/has-too-many-arguments.rs
...
/// Implementation of the `std::default::Default` trait for the `Zno` struct.
///
/// This implementation allows a `Zno` object to be created with default values.
///
/// # Examples
///
/// ```
/// let zno = Zno::default();
/// assert_eq!(zno.name.value(), "__default");
/// assert_eq!(zno.default.value(), true);
/// assert_eq!(zno.validated.value(), false);
/// assert_eq!(zno.enter_on_poll.value(), false);
/// assert_eq!(zno.scope.unwrap(), Scope::Local);
/// // ... and so on for the other fields
/// ```
///
/// # Safety
...
mod tests {
    ...
    #[test]
    fn valid_trace_001() {
        // let ts = syn::parse::Parser::parse_str(syn::Attribute::parse_outer, "#[zno]").unwrap();
        ...
        let actual = syn::parse2::<Zno>(args).unwrap();
        let expected = Zno {
    ...
    fn invalid_trace_001() {
        ...
        let actual = match syn::parse2::<Zno>(args.clone()) {
            Err(error) => error,
