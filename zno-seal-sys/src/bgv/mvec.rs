...
///
/// In the BGV encryption scheme as implemented by SEAL, the `mvec` parameter specifies a
...
/// and the exact details of the BGV implementation in SEAL, these might represent different
...
///
/// In SEAL's BGV encryption scheme, `mvec` is a parameter that holds a vector of integers
...
///
/// ## Range in SEAL:
/// The choice of values for `mvec` in SEAL impacts both the security and performance of operations.
/// Specific values determine the algebraic structure used for the ciphertexts. Users should refer to
/// SEAL's official documentation or relevant publications for detailed guidelines on selecting `mvec`.
///
/// # Example
///
/// ```
